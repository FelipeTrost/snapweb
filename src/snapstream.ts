...
class PcmChunkMessage extends BaseMessage {
  ...
  startMs(): number {
    return (
      ...
      1000 * (this.idx / this.sampleFormat.rate)
    );
  }
  ...
  addPayload(buffer: ArrayBuffer) {
    const payload = new ArrayBuffer(
      this.payload.byteLength + buffer.byteLength,
    );
...
class AudioStream {
  ...
  addChunk(chunk: PcmChunkMessage) {
    ...
    while (this.chunks.length > 0) {
      const age =
        this._timeProvider.serverNow() -
        this.chunks[0].timestamp.getMilliseconds();
...
class FlacDecoder extends Decoder {
  ...
  write_callback_fn(data: Array<Uint8Array>, frameInfo: Flac.BlockMetadata) {
    ...
    const payload = new ArrayBuffer(
      this.sampleFormat.frameSize() * frameInfo.blocksize,
    );
    const view = new DataView(payload);
    for (let channel: number = 0; channel < frameInfo.channels; ++channel) {
      const channelData = new DataView(
        data[channel].buffer,
        0,
  ...
  cacheInfo: { isCachedChunk: boolean; cachedBlocks: number } = {
    isCachedChunk: false,
    cachedBlocks: 0,
  };
}
...
class SnapStream {
  ...
  private setupAudioContext(): boolean {
    if (AudioContext) {
      ...
      if (
        (chromeVersion !== null && chromeVersion < 55) ||
        !window.AudioContext
      ) {
    ...
    if (type === 1) {
      ...
      if (this.decoder) {
        ...
        if (this.sampleFormat.channels !== 2 || this.sampleFormat.bits < 16) {
          alert(
            "Stream must be stereo with 16, 24 or 32 bit depth, actual format: " +
          ...
          this.gainNode.gain.value = this.serverSettings!.muted
            ? 0
          ...
          this.latency =
            ...
            (this.ctx.outputLatency !== undefined
      ...
      const pcmChunk = new PcmChunkMessage(
        msg.data,
      ...
      this.gainNode.gain.value = this.serverSettings.muted
        ...
        : this.serverSettings.volumePercent / 100;
      this.bufferMs =
        this.serverSettings.bufferMs - this.serverSettings.latency;
      console.log(
  ...
  public playNext() {
    ...
    const playBuffer = new PlayBuffer(
      buffer,
      this.playTime,
      source,
      this.gainNode!,
