namespace Snapcast {
  ...
  export class Client {
    ...
    fromJson(json: any) {
      ...
      this.config = {
        ...
        name: jconfig.name,
        volume: {
          muted: jconfig.volume.muted,
          percent: jconfig.volume.percent,
        },
      };
  ...
  export class Server {
    ...
    getClientGroup(id: string): Group | null {
      for (const group of this.groups) {
        for (const client of group.clients) {
          if (client.id === id) return group;
        }
      }
      return null;
    }
...
class SnapControl {
  ...
  onConnectionChanged:
    | ((_this: SnapControl, _connected: boolean, _error?: string) => any)
    | null;
  ...
  private onNotification(notification: any): boolean {
    ...
    switch (notification.method) {
      case "Client.OnVolumeChanged":
        this.getClient(notification.params.id).config.volume =
          notification.params.volume;
        // updateGroupVolume(this.getGroupFromClient(client.id));
        return true;
      case "Client.OnLatencyChanged":
        this.getClient(notification.params.id).config.latency =
          notification.params.latency;
        return false;
      case "Client.OnNameChanged":
        this.getClient(notification.params.id).config.name =
          notification.params.name;
        return true;
      case "Client.OnConnect":
      case "Client.OnDisconnect":
        this.getClient(notification.params.client.id).fromJson(
          notification.params.client,
        );
        return true;
      case "Group.OnMute":
        this.getGroup(notification.params.id).muted = Boolean(
          notification.params.mute,
        );
        return true;
      case "Group.OnStreamChanged":
        this.getGroup(notification.params.id).stream_id =
          notification.params.stream_id;
        return true;
  ...
  public getGroupFromClient(client_id: string): Snapcast.Group {
    for (const group of this.server.groups)
      for (const client of group.clients)
        if (client.id === client_id) return group;
  ...
  public setClientLatency(client_id: string, latency: number) {
    ...
    if (latency !== current_latency) {
      this.sendRequest("Client.SetLatency", {
        id: client_id,
        latency: latency,
      });
  ...
  public setClients(group_id: string, clients: string[]) {
    this.status_req_id = this.sendRequest("Group.SetClients", {
      id: group_id,
      clients: clients,
    });
